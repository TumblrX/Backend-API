...
/**
 * Is used to edit BlogPassward
 * @async
 * @function editBlogPassward
 * @param {Request} req
 * @param {Respone} res
 * @param {function} next
 */

const editBlogPassward = catchAsync(async (req, res, next) => {
    ...
    if (belong(user.blogs.indexOf(req.params.blogid), req.user.primaryBlog, req.params.blogid)) {
        if (!blog.isPrimary) {
            if (blog.settings.activatePassward) {
                const hashpassward = await bcrypt.hash(req.body.password, 10);
                blog.password = hashpassward;
                await blog.save();
                res.status(200).json({
                    status: 'success',
                });
            } else {
                return next(new AppError('you MUST activate passward', 400));
            }
        } else {
            return next(new AppError('You are NOT secondary blog', 400));
        }
    } else {
...
module.exports = {editBlogPassward, validityOfData, belong};
