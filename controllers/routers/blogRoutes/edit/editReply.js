...
const editReply = catchAsync(async (req, res, next) => {
    ...
    if (belong(user.blogs.indexOf(req.params.blogid), req.user.primaryBlog, req.params.blogid)) {
        blog.settings.replies = req.body.replies;
        await blog.save();
        res.status(200).json({
            status: 'success',
        });
...
module.exports = {editReply, validityOfData, belong};
