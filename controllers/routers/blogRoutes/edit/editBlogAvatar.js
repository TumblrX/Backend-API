...
const editBlogAvatar = catchAsync(async (req, res, next) => {
    ...
    if (belong(user.blogs.indexOf(req.params.blogid), req.user.primaryBlog, req.params.blogid)) {
        if (!blog.isPrimary) {
            blog.settings.blogAvatar = req.body.blogAvatar;
            await blog.save();
            res.status(200).json({
                status: 'success',
            });
...
module.exports = {editBlogAvatar, validityOfData, belong};
