...
/**
 * Is used to edit Messaging
 * @async
 * @function editMessaging
 * @param {Request} req
 * @param {Respone} res
 * @param {function} next
 */

const editMessaging = catchAsync(async (req, res, next) => {
    ...
    if (belong(user.blogs.indexOf(req.params.blogid), req.user.primaryBlog, req.params.blogid)) {
        blog.settings.messaging = req.body.messaging;
        await blog.save();
        res.status(200).json({
            status: 'success',
        });
...
module.exports = {editMessaging, validityOfData, belong};
