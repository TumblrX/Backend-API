...
const editUsername = catchAsync(async (req, res, next) => {
    ...
    if (belong(user.blogs.indexOf(req.params.blogid), req.user.primaryBlog, req.params.blogid)) {
        if (await isRepeatedHandle(req.body.username)) return next(new AppError('Handle is already taken', 400));
        blog.handle = req.query.username;
        await blog.save();
        res.status(200).json({
            status: 'success',
        });
...
module.exports = {editUsername, validityOfData, belong};
