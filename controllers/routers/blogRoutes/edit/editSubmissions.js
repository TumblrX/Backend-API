...
/**
 * Is used to edit Submissions
 * @async
 * @function editSubmissions
 * @param {Request} req
 * @param {Respone} res
 * @param {function} next
 */

const editSubmissions = catchAsync(async (req, res, next) => {
    ...
    if (belong(user.blogs.indexOf(req.params.blogid), req.user.primaryBlog, req.params.blogid)) {
        blog.settings.allowSubmission = req.body.allowSubmission;
        await blog.save();
        res.status(200).json({
            status: 'success',
        });
...
module.exports = {editSubmissions, validityOfData, belong};
