...
/**
 * Is used to edit ActivatePassward
 * @async
 * @function editActivatePassward
 * @param {Request} req
 * @param {Respone} res
 * @param {function} next
 */

const editActivatePassward = catchAsync(async (req, res, next) => {
    ...
    if (belong(user.blogs.indexOf(req.params.blogid), req.user.primaryBlog, req.params.blogid)) {
        if (!blog.isPrimary) {
            blog.settings.activatePassward = req.body.activatePassward;
            await blog.save();
            res.status(200).json({
                status: 'success',
            });
...
module.exports = {editActivatePassward, validityOfData, belong};
