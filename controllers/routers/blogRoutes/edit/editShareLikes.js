...
/**
 * Is used to edit ShareLikes
 * @async
 * @function editShareLikes
 * @param {Request} req
 * @param {Respone} res
 * @param {function} next
 */

const editShareLikes = catchAsync(async (req, res, next) => {
    ...
    if (belong(user.blogs.indexOf(req.params.blogid), req.user.primaryBlog, req.params.blogid)) {
        if (blog.isPrimary) {
            blog.settings.shareLikes = req.body.shareLikes;
            await blog.save();
            res.status(200).json({
                status: 'success',
            });
...
module.exports = {editShareLikes, validityOfData, belong};
